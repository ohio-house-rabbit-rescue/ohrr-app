...
//
// On Home (`notifyPrompt`), the area also asks once — only people who haven't
// decided, where it can work — "Get a notification when OHRR needs
// volunteers?" (update 32; PhoneNotifications.tsx).
import { Link } from 'react-router-dom'
import { Icon } from '../../components/icons'
import { markSeen, useForYou } from './forYouCounts'
import { NotificationsPrompt } from './PhoneNotifications'

export default function ForYou({ className = '', notifyPrompt = false }: { className?: string; notifyPrompt?: boolean }) {
  const { items } = useForYou()
  const prompt = notifyPrompt ? <NotificationsPrompt className={className} /> : null
