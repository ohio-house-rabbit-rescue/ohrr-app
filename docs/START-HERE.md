...
| **Database changes to paste** | Drive "OHRR App Design" → `APPLY-*.sql` (each mirrors `supabase/migrations/*`; paste into Supabase → SQL Editor, in order: `APPLY-ALL-2026-09-17`, `APPLY-2-SCAN-ITEMS`, `APPLY-3-INBOX-BOOKINGS-PAGES`, `APPLY-4-POSTS-REACH`, `APPLY-5-HOURS-IMPACT`, `APPLY-6-PUBLIC-SHOP`, `APPLY-7-RAFFLE-TICKETS`, `APPLY-8-BOOKING-SCHEDULE`, `APPLY-9-HOPSHOP-SUPPLIERS`, `APPLY-10-BUNFEST-CONTENT`, `APPLY-11-TAILS-UPLOADS-PROFILE`, `APPLY-12-VOLUNTEERS-AND-EVENT-CONTENT`) | ✅ cloud | the schema the app expects |
