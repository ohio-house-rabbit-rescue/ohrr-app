...
- **Branch:** `main` — clean, up to date with `origin/main` (HEAD `f01df57`).
- **Open PRs:** none. All 8 PRs to date are merged/closed; every feature branch
  has been merged into `main` and deleted on the remote.
- **Build health:** `npm run typecheck` passes clean.
